...
import { PiInfoLight, PiFile, PiChat } from "react-icons/pi";
...
const menuGroups = [
  {
    ...
    menuItems: [
      ...
      {
        icon: (
            <PiBuildingApartmentLight className={"fill-current"} size={24}/>
      ...
      {
        icon: (
            <PiChat  className={"fill-current"} size={24}/>
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      className={`absolute left-0 top-0 flex h-screen w-64 flex-col overflow-y-hidden border-r border-stroke bg-white dark:border-stroke-dark dark:bg-gray-dark lg:static lg:translate-x-0 ${
        sidebarOpen
